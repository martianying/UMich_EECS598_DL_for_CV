...
def compute_distances_two_loops(x_train, x_test):
  ...
  dists = x_train.new_zeros(num_train, num_test) # same dytype and same device as x_train.
  ##############################################################################
  ...
  for i in range(num_train):
    for j in range(num_test):
      ...
      square = torch.pow(x_train[i] - x_test[j], 2)
      dist_ij = torch.sqrt(torch.sum(square))
      dists[i][j] = dist_ij
...
def compute_distances_one_loop(x_train, x_test):
  ...
  for i in range(num_train):
    tensor_i = x_train[i] - x_test
    square = torch.pow(tensor_i, 2)
    dists_i = torch.sqrt(torch.sum(square, axis = 1))
...
def compute_distances_no_loops(x_train, x_test):
  ...
  x_train_2 = torch.sum(torch.pow(x_train, 2), axis = 1, keepdim = True)
  x_test_2 = torch.sum(torch.pow(x_test, 2), axis = 1)

  cross_term = 2 * torch.mm(x_train, x_test.T)

  dists = torch.sqrt(x_train_2 + x_test_2 - cross_term)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def predict_labels(dists, y_train, k=1):
  ...
  for i in range(num_test): 
    # get the topk minimums indeces
    topk_labels = torch.topk(dists[:, i], k, dim = 0, largest=False).indices

    # grab the associated labels from y_train
    pool = y_train[topk_labels]

    # get the most frequent element 
    y_pred[i] = torch.mode(pool).values.item()
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  for k in k_choices:
    ...
    for i in range(num_folds):
      ...
      train_x = list(x_train_folds)
      del train_x[i]
      train_x = torch.cat(tuple(train_x), axis=0)

      train_y = list(y_train_folds)
      del train_y[i]
      train_y = torch.cat(tuple(train_y), axis=0)     

      valid_x = x_train_folds[i]
      valid_y = y_train_folds[i]

      # training 
      trainer = KnnClassifier(train_x, train_y)

      # validating
      acc_ki = trainer.check_accuracy(valid_x, valid_y, k, quiet=True) 

      # store in k_to_accuracies dic
      acc.append(acc_ki)
