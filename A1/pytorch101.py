...
def count_tensor_elements(x):
  ...
  # convert the torch.shape to a tensor
  tensor_xshape = torch.tensor(xshape)

  # get the number of elements by multiplying all the elements in tensor_shape
  # convert the output tensor to integer
  num_elements = torch.prod(tensor_xshape, axis=0).item()
...
def slice_assignment_practice(x):
  ...
  # strategy: create a tensor that equals to the (4, 6) tensor
  # replace the target part of x by created tensor

  # create a tensor shaped (4, 6) full of twos
  orig = torch.full((4, 6), 2)
  # 1st slicing: fill 0
  orig[:2, 0] = 0
  # 2nd slicing: fill 1
  orig[:2, 1] = 1
...
def reverse_rows(x):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  x_idx0 = torch.arange(-1, -x.shape[0] - 1, -1) # alternatively: x_idx0 = torch.multiply(torch.arange(1, x.shape[0] + 1), -1)
  y = x[x_idx0]
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
...
def zero_row_min(x):
  ...
  # find the index of the min for each row
  minimum_ids = torch.argmin(y, axis=1)

  # replacing minimums by zeros using indexing 
  y_idx0 = [i for i in range(y.shape[0])]
  y[y_idx0, minimum_ids] = 0
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return y
...
def batched_matrix_multiply(x, y, use_loop=True):
  ...
  if use_loop == True:
    ...
    for i in range(x.shape[0]):
      z[i] = torch.mm(x[i], y[i]) # use .mm since it keeps axis.
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return z
...
def normalize_columns(x):
  ...
  # Replace "pass" statement with your code
  # get the mean tensor
  means = torch.div(torch.sum(x, axis=0), x.shape[0])

  # get the variance tensor
  sqrt = torch.pow(x - means, 2)
  sum_sqrt = torch.sum(sqrt, axis=0)
  stds = torch.sqrt(torch.div(sum_sqrt, x.shape[0] - 1))

  # do normalization: where broadcasting happens twice 
  y = (x - means) / stds
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
